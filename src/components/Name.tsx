...
export const Name: React.FC<NameProps> = ({ id }) => {
  ...
  return (
    <div id={id || "name-component"} className="text-center items-center text-center justify-center mb-8 animate-fade-in-up">
      <div id="name-container" className="inline-block bg-vscode-tertiary/30 backdrop-blur-sm border border-themed rounded-xl p-8 shadow-lg">
        <div id="name-terminal-prompt" className="flex items-center justify-center mb-4 text-sm font-mono text-secondary-themed">
          <span id="name-prompt-user" className="text-vscode-muted font-semibold">bhakti@portfolio</span>
          ...
          <span id="name-prompt-command" className="ml-2 success-">whoami</span>
        </div>
        ...
        <div id="name-subtitle" className="mt-4 text-secondary-themed font-mono text-sm">
          <span id="name-check-mark" className="text-success-themed">✓</span>
          <span id="name-role" className="ml-2">Backend Developer | AI Enthusiast</span>
        </div>
      </div>
    </div>
  );
