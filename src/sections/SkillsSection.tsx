...
import { flattenedSkills } from '../data/portfolio';
...
const SkillsSection: React.FC = () => {
  ...
  const filteredSkills = useMemo(() => {
    let filtered = flattenedSkills.filter(skill => {
  ...
  const getCategoryCount = useCallback((category: string) => {
    if (category === "All Categories") return flattenedSkills.length;
    return flattenedSkills.filter(skill => skill.category === category).length;
  }, []);

  const getLevelCount = useCallback((level: string) => {
    if (level === "All Levels") return flattenedSkills.length;
    const levelInfo = skillLevelScale.find(l => l.label === level);
    return levelInfo ? flattenedSkills.filter(skill => skill.score === levelInfo.level).length : 0;
  }, []);
  ...
  const groupedSkills = useMemo(() => {
    ...
    const uniqueCategories = Array.from(new Set(
      flattenedSkills.map(skill => skill.category)
    ));
